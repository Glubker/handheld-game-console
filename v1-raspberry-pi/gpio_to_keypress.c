...
void start_or_relaunch_game_console() {
    char command[256];
    int retries = 5; // Number of retries to wait for process termination

    // Kill any running games in the /home/pi/game_console/games directory
    system("pkill -f /home/pi/game_console/games");

    // Kill existing instances of the game console
    system("pkill -f /home/pi/game_console/build/game_console");

    // Wait for all processes to terminate completely
    while (retries > 0) {
        // Check if any games or the game console process is still running
        if (system("pgrep -f '/home/pi/game_console/games' > /dev/null") != 0 &&
            system("pgrep -f '/home/pi/game_console/build/game_console' > /dev/null") != 0) {
            // No relevant processes are running
            break;
        }
    ...
    // If retries ran out, forcefully kill any remaining processes
    if (retries == 0) {
        system("pkill -9 -f /home/pi/game_console/games");
        system("pkill -9 -f /home/pi/game_console/build/game_console");
    }
